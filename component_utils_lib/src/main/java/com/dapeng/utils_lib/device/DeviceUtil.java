...
public final class DeviceUtil {
            ...
            try {
                ...
                if (obj_get != null && obj_get instanceof View) {
                    ...
                    if (v_get.getContext() == destContext) { // 被InputMethodManager持有引用的context是想要目标销毁的
                        ...
                        DPLogUtils.debugLevel(TAG, "fixInputMethodManagerLeak break, context is not suitable, get_context=" + v_get.getContext() + " dest_context=" + destContext);
                        break;
                    }
                }
            } catch (Throwable t) {
    ...
    public static void showInputMethodDelay(final View view, final Activity activity, int delay) {
        // 显示输入法
        ThreadUtils.runOnUiThreadDelay(new Runnable() {
            @Override
            public void run() {
                if (view != null&&!activity.isDestroyed()) {
    ...
    public static void gotoNotificationSetting(Activity activity, int requestCode) {
        ...
        try {
            ...
            DPLogUtils.errorLevel(TAG, e.getMessage(), e);
        }
    }
